...
export default function WeddingGallery() {
  ...
  const prev = () =>
    setLightboxIndex(
      (i) => (i! - 1 + weddingImages.length) % weddingImages.length,
    );
    ...
    <div className="min-h-screen bg-[#F5F1E8] text-[#2C2416]">
      ...
      <header className="py-24 px-8 md:px-16 max-w-[1400px] mx-auto">
        ...
        >
          ...
          <h1
            className={`text-6xl md:text-8xl font-light mt-4 mb-6 leading-none ${cormorant.className}`}
          >
            Wedding
            <br />
          ...
          <p
            className={`text-lg md:text-xl leading-relaxed max-w-2xl text-[#4A3F2E] ${tenorSans.className}`}
          >
            Every wedding is a universe of its own — emotion layered on emotion,
            light shifting through moments that will never come again. We dont
            just photograph weddings; we preserve them. Our team brings over a
            decade of experience crafting images that feel both timeless and
            deeply personal. From the quiet morning preparations to the last
            dance, we are there, invisible yet ever-present, capturing what
            words cannot hold.
          </p>
        </motion.div>
      </header>
      ...
      <section className="py-24 px-8 md:px-16 bg-[#2C2416] text-center">
        <p
          className={`text-xs tracking-[0.3em] text-[#8B7355] mb-4 ${tenorSans.className}`}
        >
          READY TO BEGIN?
        </p>
        <h2
          className={`text-4xl md:text-5xl font-light text-[#F5F1E8] mb-8 ${cormorant.className}`}
        >
